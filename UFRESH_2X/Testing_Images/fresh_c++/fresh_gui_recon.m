...
function [ I_rec ] = fresh_plus_reconst(block, factor, diag_reg, lin_map, high_res_in, profile)


    I = fresh(block, log2(factor), ...
    ...
    if all(point>=block_size) && all(point<=[size(img,2) size(img,1)])
        ...
        if ~isequal(last_ups_block,point)
            ...
            if strcmp(event.EventName,'WindowMousePress')
                ...
                pause(eps),drawnow
                tic;
                block_ups = fresh_plus_reconst(block, log2(level), ...
                    diag_reg, lin_map, high_res_in, profile);
                elapsed_time = toc;
                ...
                if truesize_img
                    ...
                    imshow(uint8(block), [0 255], 'Parent',hAxLR)
                    truesize(fLR)
