...
n = [16384];
filtvec = {'bior4.4','bior6.8','rbio2.8','haar','db2','db4'};
meanpsnrs = zeros(length(filtvec),1);
meanssims = zeros(length(filtvec),1);
meantimeperpixel = zeros(length(filtvec),1);

for fnum = 1:length(filtvec)
    filt = filtvec{fnum};
    fprintf('################   %s    #####################\n',filt)
...
%         visualiseImprovement(Xcell{imgIdx}, Xtest, Ytest)
    ...
    meanpsnrs(fnum) = mean(postpsnr);
    meanssims(fnum) = mean(postssim);
end

plt = 1;
if plt
    % Plot results
    nvals = 1:length(filtvec);
    figure;
