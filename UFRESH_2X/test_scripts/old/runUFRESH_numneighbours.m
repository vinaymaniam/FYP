...
for nn = numneighbours
    ...
    for imgIdx = 1:length(Xcell)
        ...
        for stage = 1:1
            ...
            load(sprintf('%ipyMap%icell%i',stage,n,nn));
...
%         visualiseImprovement(Xcell{imgIdx}, Xtest, Ytest)
    ...
    meanpsnrs(find(numneighbours==nn)) = mean(postpsnr);
    meanssims(find(numneighbours==nn)) = mean(postssim);
    meantimeperpixel(find(numneighbours==nn)) = mean(tpp);    
end
...
if plt
    ...
    figure;
    plot(numneighbours, meantimeperpixel,'o--')
    xlabel('Number of Centroids')
