function [I_rec, IDX, MSE]=FRI_errRepair_ann_lm2(I,I_fri,I_fri2,level,IDX,MSE)
size_I=size(I_fri2);
...
for xx=ceil(num*ratio)+p_size+1:size_I(1)+ceil(num*ratio)-p_size
    for yy=ceil(num*ratio)+p_size+1:size_I(2)+ceil(num*ratio)-p_size
      ...
      [x_p,y_p]=ind2sub(size(Region_fri)-patchsize(1)+1,idx);
         ...
         %%%%%%%%%%%
%          for ss=1:length(x_p)
%             x_ref=round((x_p(ss)+2)*ratio)+(-p_size:p_size);
%             y_ref=round((y_p(ss)+2)*ratio)+(-p_size:p_size);
