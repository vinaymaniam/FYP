...
%          if ss==200
%          figure,plot(x_f,'k'),hold on, plot(I_lin(ss,:),'g'),hold on,plot(x_rec,'r'),hold on,stem(inloc_est*NN2+NN2./2,20*ones(size(inloc_est)));%hold on,stem(1:2^level:NN,y_n./4,'g')
%          end
    Coef_x=[Coef_x;x_rec(:).'];
end
...
%    figure(5),hold on,imshow(Coef_d1,[]);
end
...
%I_up1to2{it}=intp_bic(I_up1to2{it},2/b);
% I_tmp=intp_bic(I_rec,b);
% [c1,l1] = wavedec2(I_tmp,1,filter);
% I_up1to2{it}=appcoef2(c1,l1,filter,1)./2;
end
end
I_upres{it}=I_rec;
end
%I_rec=I_upres{it}(5:end-4,5:end-4);I=I(5:end-4,5:end-4);
if atf
imagename
bd=4;PSNR_rec=PSNR(I_rec(bd+1:end-bd,bd+1:end-bd),I(bd+1:end-bd,bd+1:end-bd),255)
...
figure,imshow(I_c,[]);imwrite(uint8(I_c),'temp.bmp');
else
    figure,imshow(I_rec,[]);
